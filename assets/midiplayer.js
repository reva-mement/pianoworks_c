...
var Player = /*#__PURE__*/function () {
  ...
  _createClass(Player, [{
      ...
      if (!this.inLoop) {
        ...
        this.tracks.forEach(function (track, index) {
          ...
          if (!dryRun && this.endOfFile()) {
            ...
            // ★ dryRun（事前解析パス）は元の挙動のまま変更しない
            var _dryEvent = track.handleEvent(this.tick, dryRun);

            if (_dryEvent) {
              if (_dryEvent.hasOwnProperty('name') && _dryEvent.name === 'Set Tempo') {
                this.defaultTempo = _dryEvent.data;
                this.setTempo(_dryEvent.data);
              }

              if (_dryEvent.hasOwnProperty('name') && _dryEvent.name === 'Program Change') {
                if (!this.instruments.includes(_dryEvent.value)) {
                  this.instruments.push(_dryEvent.value);
                }
              }
            }
          } else {
            // ★★★ 修正：以前はここで track.handleEvent() を1回しか呼んでおらず、
            //   1回のループ（sampleRate=5ms間隔）につきイベントを1個しか処理できなかった。
            //   そのため、同じ瞬間（同じtick、もしくはこの5msの間に複数溜まったtick）に
            //   本来同時であるべきイベントが複数ある場合、後回しにされたイベントは
            //   次の5ms・その次の5ms……とループを跨いでようやく処理される形になり、
            //   同時に鳴るはずの音や、ブロックとして出るはずの音が数十ミリ秒単位で
            //   遅延する原因になっていた。
            //   ここを while ループにし、このtickまでに溜まっている該当イベントを
            //   このループ内でなくなるまで全部処理しきるようにする（実再生時のみ）。
            var event;

            while (event = track.handleEvent(this.tick, dryRun)) {
     ...
     * @return {Player}
     */
// --- midiplayer.js の 911行目あたり（skipToTick の終了 } の直後）に挿入 ---

    /**
     * 再生状態を維持したまま、指定したTickへ瞬間移動する（Studioモード用）
     */
    }, {
    key: "jumpToTick",
    value: function jumpToTick(tick) {
      // stop() を呼ばないことでポインタのリセットを防ぐ
      this.startTick = tick;
      this.tick = tick;

      // 再生中の場合は、基準時刻(startTime)を「今」に更新して計算を合わせる
      if (this.isPlaying()) {
        this.startTime = new Date().getTime();
      }

      // 各トラックの読み込みインデックスを、指定したTickの直近までジャンプさせる
      this.tracks.forEach(function (track) {
        track.setEventIndexByTick(tick);
      });
      return this;
